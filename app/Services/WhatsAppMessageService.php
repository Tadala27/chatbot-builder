...
class WhatsAppMessageService
{
    ...
    public function sendMediaMessage(
        ...
        if (!empty($caption))  $mediaObject['caption']   = $caption;
        if (!empty($filename)) $mediaObject['filename']  = $filename;
        if (!empty($mimeType)) $mediaObject['mime_type'] = $mimeType;
    ...
    public function sendMediaFile(
        ...
        if (!empty($caption)  && $mediaType !== 'audio') $mediaObject['caption']  = $caption;
        if ($mediaType === 'document')                    $mediaObject['filename'] = $file->getClientOriginalName();
        ...
        if ($replyToWamid) $content['context'] = ['message_id' => $replyToWamid];
    ...
    public function sendTypingIndicator(
        ...
        $lastInbound = \App\Models\Message::join('conversations', 'conversations.id', '=', 'messages.conversation_id')
        ...
        // Include typing_indicator only when starting to type.
        // Omitting it acts as "stop typing" per the API spec.
        if ($isTyping) {
    ...
    private function assertAccessToken(WhatsappAccount $account): void
    {
        if (empty($account->access_token)) {
            throw new \RuntimeException(
                "WhatsApp account (id={$account->id}) has no access_token. Please reconnect in Settings."
            );
        }
    }
    ...
    private function addSystemVariables(array $variables, string $to, WhatsappAccount $account): array
    {
        ...
        try {
            $conv     = Conversation::where('whatsapp_account_id', $account->id)
                ->where('whatsapp_user_phone', $to)->latest()->first();
     ...
     */
    private function detectMediaType(\Illuminate\Http\UploadedFile $file): string
    {
