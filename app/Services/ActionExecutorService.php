...
class ActionExecutorService
{
    ...
    private function storeResponseAsVariables(
        Conversation $conversation,
        array        $declaredParams,   // body_parameters or header_parameters (decoded)
        array        $responseBody,
        int          $botId
    ): void {
        ...
        // Build a lookup of custom_variable key → id for this bot, covering only
        // the keys we actually need (avoids a full table scan).
        $paramKeys = [];
        foreach ($declaredParams as $param) {
            // Params are stored as ["{{memberName}}", ...] — strip the braces.
        ...
        // Single query for all relevant custom variables.
        $customVars = \App\Models\CustomVariable::where('bot_id', $botId)
            ->whereIn('key', $paramKeys)
            ->pluck('id', 'key'); // ['memberName' => 5, 'DoJ' => 7, ...]

        foreach ($paramKeys as $key) {
            // The response body key must exist (even if the value is null/0/false).
            if (!array_key_exists($key, $responseBody)) {
                continue;
            }

            $value          = $responseBody[$key];
            $customVariableId = $customVars->get($key); // null if not in custom_variables
            ...
            Log::info('API response variable stored', [
                'conversation_id'    => $conversation->id,
                'key'                => $key,
                'value'              => $scalarValue,
                'custom_variable_id' => $customVariableId,
