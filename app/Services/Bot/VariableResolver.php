...
class VariableResolver
{
    ...
    private function resolveFunctionCalls(string $text, array $variables): string
    {
        ...
        while ($i < $len) {
            ...
            if ($text[$i] === '$' && $i + 1 < $len && $this->isIdentStart($text[$i + 1])) {
                // Try to match $name(
                $j = $i + 1;
                while ($j < $len && $this->isIdentChar($text[$j])) $j++;
    ...
    // =========================================================================
    // BUILT-IN FUNCTIONS — unchanged from your version
    // =========================================================================
    ...
    {
        foreach ($params as $p) if (!$this->isTruthy($p)) return 'false';
        return 'true';
    }

    private function evaluateOr(array $params): string
    {
        foreach ($params as $p) if ($this->isTruthy($p)) return 'true';
        return 'false';
    }
}