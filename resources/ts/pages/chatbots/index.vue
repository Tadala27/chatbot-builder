...
const filteredFlows = computed(() => {
  ...
  if (statusFilter.value !== "all") {
    filtered = filtered.filter(
      (flow: any) => flow.status === statusFilter.value,
    );
  }
    ...
    <VRow class="mb-4">
      <VCol cols="12">
        <div class="d-flex align-center justify-space-between flex-wrap gap-3">
          <div>
            <h2 class="text-h4 mb-2">Chatbots</h2>
            <p class="text-medium-emphasis">Manage your WhatsApp Chatbos</p>
          </div>
          <VBtn
            color="primary"
            prepend-icon="$plus"
    ...
    <VRow v-else>
      ...
      >
        <VCard elevation="2" hover>
          ...
          <VCardText>
            <div v-if="flow.description" class="mb-4" style="min-height: 60px">
              <div
                v-html="flow.description"
                class="text-caption text-medium-emphasis line-clamp-3"
              ></div>
            </div>
            ...
            <VRow dense class="text-caption">
              <VCol cols="6">
                <div class="d-flex align-center">
                  ...
                  />
                  <span>{{
                    flow.whatsapp_account?.display_phone_number || "N/A"
                  }}</span>
                </div>
              </VCol>
              <VCol cols="6">
                <div class="d-flex align-center">
                  ...
                  <span
                    >{{
                      flow.stats?.total_conversations || 0
                    }}
                    conversations</span
                  >
                </div>
              ...
              <VCol cols="6" class="mt-2">
                <div class="d-flex align-center">
                  <VIcon icon="$clock" size="16" color="info" class="mr-1" />
                  <span class="text-disabled">
                    Updated {{ moment(flow.updated_at).fromNow() }}</span
                  >
                </div>
              </VCol>
            </VRow>
          ...
          <VCardActions class="pa-4">
            <VBtn
              ...
              size="small"
              prepend-icon="$pencil"
              :to="`/chatbots/flowbuilder/${flow.id}`"
            >
              Edit Flow
            </VBtn>
            <VSpacer />
            <VMenu>
              ...
              <VList>
                <VListItem prepend-icon="$eye" :to="`/flows/${flow.id}`">
                  View Details
                </VListItem>
                <VListItem
                  prepend-icon="$chartTimelineVariant"
