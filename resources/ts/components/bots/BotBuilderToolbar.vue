...
const currentVersion = computed(() =>
  props.botVersions.find((v) => v.id === props.selectedVersionId),
);
...
<template>
  <VToolbar density="compact" border="b" elevation="0" color="surface">
    ...
    <VChip
      ...
      variant="tonal"
      size="small"
      :prepend-icon="saveIndicatorIcon"
      class="mr-2"
    >
      {{ saveIndicatorText }}
    </VChip>
    ...
    <VMenu v-if="botVersions.length > 0">
      ...
      <VCard min-width="320">
        <VCardTitle
          class="text-subtitle-2 bg-grey-lighten-4 d-flex align-center justify-space-between"
        >
          <div>
          ...
          <VBtn
            ...
            variant="text"
            color="primary"
            prepend-icon="$sourceBranchPlus"
            @click="emit('createNewVersion')"
            :disabled="
              botVersions.find((v) => v.id === selectedVersionId)?.status ===
              'draft'
            "
          >
            New Version
        ...
        <VCardText class="pa-0">
          <VList density="compact" max-height="400" style="overflow-y: auto">
            <VListItem
              ...
              :class="{
                'bg-primary-lighten-5': version.id === selectedVersionId,
              }"
            >
              <template #prepend>
                <VChip
        ...
        <VCardText
          class="pa-2 text-caption text-grey-darken-1 bg-grey-lighten-5"
        >
          <VIcon icon="$sourceBranchPlus" size="x-small" class="mr-1" />
          Create a new version to work on
        </VCardText>
      </VCard>
    </VMenu>

    <VBtn
      v-if="botVersion?.status !== 'published'"
      color="primary"
      size="small"
      rounded="lg"
