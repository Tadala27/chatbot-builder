...
<template>
  <div>
    <v-row>
      <v-col cols="6">
        <v-text-field v-model.number="node.locationLatitude" label="Latitude" placeholder="-25.7479" variant="outlined"
          density="compact" type="number" step="any" hint="Example: -25.7479" persistent-hint>
      ...
      <v-col cols="6">
        <v-text-field v-model.number="node.locationLongitude" label="Longitude" placeholder="28.2293" variant="outlined"
          density="compact" type="number" step="any" hint="Example: 28.2293" persistent-hint>
          <template #prepend-inner>
    ...
    <v-text-field v-model="node.locationName" label="Location Name" placeholder="OneNICO Office" variant="outlined"
      density="compact" class="mt-2" hint="Display name for the location" persistent-hint>
      <template #prepend-inner>
        <v-icon icon="$mapMarker" size="small" />
      </template>
    </v-text-field>

    <v-textarea v-model="node.locationAddress" label="Address (optional)" placeholder="123 Main Street, City"
      variant="outlined" density="compact" rows="2" class="mt-3" hint="Full address of the location" persistent-hint>
      <template #prepend-inner>
    ...
    <v-card variant="tonal" class="mb-3">
      <v-card-text class="text-caption">
        ...
        <div v-if="node.locationLatitude && node.locationLongitude" class="text-grey">
          {{ node.locationLatitude }}, {{ node.locationLongitude }}
        </div>
        <div v-if="node.locationAddress" class="mt-1">
          {{ node.locationAddress }}
        </div>
        <v-alert v-if="!node.locationLatitude || !node.locationLongitude" type="warning" variant="tonal"
