...
/**
 * components/InteractiveOptionsDialog.vue
 *
 ...
 */
import { ref } from "vue";
import type { InteractiveOption } from "@/utils/messagePreview";
    ...
    <div v-if="open" class="iod__overlay" @click.self="close">
      <div class="iod__panel" role="dialog" aria-modal="true">
        <header class="iod__header">
          <span class="iod__title">{{ kind === "list" ? "List message" : "Button message" }}</span>
        ...
        <div class="iod__body">
          <p v-if="header" class="iod__lead iod__lead--header">{{ header }}</p>
          <p v-if="body" class="iod__lead">{{ body }}</p>
          ...
          <ul class="iod__options">
            ...
            >
              ...
              <span v-if="option.description" class="iod__option-desc">{{ option.description }}</span>
            </li>
          </ul>

          <p v-if="footer" class="iod__footer">{{ footer }}</p>
          <p class="iod__note">This is a read-only preview of what was sent to the contact. Selecting an option here doesn't send anything.</p>
        </div>
      </div>
    </div>
  </Teleport>
